...
func spawn_players(country: String, own_goal: Goal) -> Array[Player]:
	...
	for i in players.size():
		...
		var player: Player = spawn_player(player_position, own_goal, target_goal, player_data, country, kickoff_position)
		player_nodes.append(player)
		add_child(player)
	return player_nodes


func spawn_player(player_pos: Vector2, own_goal: Goal, target_goal: Goal, player_data: PlayerResource, country: String, kickoff_pos: Vector2) -> Player:
	var player: Player = PLAYER_PREFAB.instantiate()
	player.initialize(player_pos, ball, own_goal, target_goal, player_data, country, kickoff_pos)
...
func on_player_swap_requested(requester: Player) -> void:
	...
	if closest_cpu_to_ball.position.distance_squared_to(ball.position) < requester.position.distance_squared_to(ball.position):
		var control_scheme := requester.control_scheme
		requester.set_control_scheme(Player.ControlScheme.CPU)
		closest_cpu_to_ball.set_control_scheme(control_scheme)
...
func on_impact_received(impact_pos: Vector2, _is_high_impact: bool) -> void:
