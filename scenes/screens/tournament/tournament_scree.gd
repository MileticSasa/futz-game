...
func refresh_bracket_stage(stage: Tournament.Stage) -> void:
	var flag_nodes := get_falg_nodes_for_stage(stage)
	...
	if stage < Tournament.Stage.COMPLETE:
		...
		assert(flag_nodes.size() == matches.size() * 2)
		for i in range(matches.size()):
			...
			flag_away.texture = FlagHelper.get_texture(current_match.county_away)
			...
			elif [current_match.country_home, current_match.county_away].has(player_country) and stage == tournament.current_stage:
...
func get_falg_nodes_for_stage(stage: Tournament.Stage) -> Array[BracketFlag]:
	var flag_nodes: Array[BracketFlag] = []
	for conatainer in flag_containers.get(stage):
		for node in conatainer.get_children():
